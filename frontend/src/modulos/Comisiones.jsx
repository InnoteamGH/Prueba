    ...
    <div style={{ display: "grid", gap: 16 }}>
      ...
      <div className="dc-split">
        <Card style={{ padding: "18px 20px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}><TrendingUp size={17} strokeWidth={1.75} color={DS.c.primary} /><span style={{ fontWeight: 600, color: NAVY, fontSize: 15 }}>Tendencia de producción</span></div>
              ...
              <div style={{ display: "flex", alignItems: "flex-end", gap: 14, height: 132 }}>
                {trend.map((t, i) => { const last = i === trend.length - 1; return (
                  <div key={`${t.m}-${i}`} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 7, height: "100%", justifyContent: "flex-end" }}>
                    <span style={{ fontSize: 12, fontWeight: 600, color: last ? DS.c.primary : "var(--dc-ink-500)", fontVariantNumeric: "tabular-nums" }}>{t.v >= 1000 ? `${(t.v / 1000).toFixed(1)}k` : t.v}</span>
        ...
        <Card style={{ padding: "18px 20px", display: "flex", flexDirection: "column" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}><CalendarCheck size={17} strokeWidth={1.75} color={TEAL} /><span style={{ fontWeight: 600, color: NAVY, fontSize: 15 }}>Meta global del mes</span></div>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 10 }}>
            <span style={{ fontSize: 32, fontWeight: 700, color: pctMetaGlobal == null ? "var(--dc-ink-400)" : NAVY, fontFamily: DISPLAY_FONT, lineHeight: 1 }}>{pctMetaGlobal == null ? "—" : pctMetaGlobal + "%"}</span>
            <span style={{ fontSize: 13, color: "var(--dc-ink-400)", fontWeight: 600 }}>{pctMetaGlobal == null ? "sin meta que comparar" : "alcanzado"}</span>
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "var(--dc-ink-400)", fontWeight: 600, marginTop: "auto", paddingTop: 12 }}>
      ...
      <DataTable titulo="Comisiones por odontólogo" sub="odontólogos" minWidth={640} rows={data} onRowClick={(m) => setDetalle(m)} empty={<Vacio icon={<Percent size={22} strokeWidth={1.75} />} titulo="Sin producción" sub="Aún no hay atenciones facturadas este mes." />} cols={[
        { key: "od", label: "Odontólogo", w: "minmax(200px,1.1fr)", a: "left", get: (m) => m.nombre, cell: (m) => <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}><div style={{ width: 34, height: 34, borderRadius: "var(--dc-r-full)", background: m.color, color: "var(--dc-white)", display: "grid", placeItems: "center", fontWeight: 600, fontSize: 12, flexShrink: 0 }}>{m.foto}</div><span style={{ fontWeight: 600, color: NAVY, fontSize: 15, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{m.nombre}</span>{m.prod === maxProd && <span style={{ fontSize: 12, fontWeight: 600, color: "var(--dc-warn-600)", background: "var(--dc-warn-soft)", padding: "2px 7px", borderRadius: "var(--dc-r-full)", flexShrink: 0 }}>TOP</span>}</div> },
        { key: "n", label: "Atenciones", w: "120px", a: "right", get: (m) => m.n, cell: (m) => <span style={{ fontSize: 15, fontWeight: 600, color: NAVY, fontVariantNumeric: "tabular-nums" }}>{m.n}</span> },
        { key: "prod", label: "Producción", w: "minmax(120px,1fr)", a: "right", get: (m) => m.prod, cell: (m) => <span style={{ fontSize: 15, fontWeight: 700, color: NAVY, fontFamily: DISPLAY_FONT, fontVariantNumeric: "tabular-nums" }}>S/ {m.prod.toLocaleString()}</span> },
        { key: "com", label: "Comisión", w: "minmax(120px,0.9fr)", a: "right", get: (m) => m.com, cell: (m) => <span style={{ fontWeight: 700, color: RED, fontFamily: DISPLAY_FONT, fontSize: 15 }}>S/ {m.com.toLocaleString()}</span> },
        ...
        { key: "meta", label: "Meta mensual", w: "minmax(190px,2fr)", a: "left", get: (m) => (m.pct == null ? -1 : m.pct), cell: (m) => {
          ...
          return <div style={{ minWidth: 0 }}><div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}><span style={{ color: "var(--dc-ink-500)" }}>meta S/ {m.meta.toLocaleString()}</span><span style={{ fontWeight: 600, color: c }}>{m.pct}%</span></div><div style={{ height: 6, background: "var(--dc-line)", borderRadius: "var(--dc-r-full)", overflow: "hidden" }}><div style={{ width: Math.min(100, m.pct) + "%", height: "100%", background: c, borderRadius: "var(--dc-r-full)", transition: "width .9s cubic-bezier(.2,.7,.2,1)" }} /></div></div>; } },
      ]} />
      {editMetas && (
          ...
          <div style={{ display: "grid", gap: 10 }}>
            {data.map((m) => (
              <label key={m.id} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ flex: 1, minWidth: 0, fontSize: 13, fontWeight: 600, color: NAVY, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.nombre}</span>
                ...
                <input className="dc-premium-inp" type="number" min="0" step="100" placeholder="sin meta"
                  ...
                  style={{ width: 120, padding: "9px 11px", borderRadius: "var(--dc-r-md)", border: "1.5px solid var(--dc-line)", fontSize: 15, textAlign: "right", fontFamily: "inherit", outline: "none" }} />
              </label>
            ))}
          </div>
        </Modal>
      ...
      {detalle && (() => { const m = detalle; const esp = ESPECIALIDADES.find((e) => e.id === m.esp); const subEsp = m.espNombre || (esp ? esp.nombre : "Odontólogo"); return (
        <Modal icon={<span style={{ fontWeight: 600 }}>{m.foto}</span>} tone={m.color} titulo={m.nombre} sub={subEsp} onClose={() => setDetalle(null)} maxW={500} footer={<Btn small kind="ghost" onClick={() => setDetalle(null)}>Cerrar</Btn>}>
